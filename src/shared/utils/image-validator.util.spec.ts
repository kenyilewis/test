...
describe('Image Validator Utilities', () => {
  describe('validateImageFile', () => {
    ...
    it('should validate a valid image file', async () => {
      ...
      await expect(
        validateImageFile('/path/to/image.jpg'),
    ...
    it('should throw InvalidImageFormatException if file does not exist', async () => {
      ...
      await expect(
        validateImageFile('/path/to/nonexistent.jpg'),
    ...
    it('should throw InvalidImageFormatException if Sharp fails to read metadata', async () => {
      ...
      await expect(validateImageFile('/path/to/invalid.txt')).rejects.toThrow(
        InvalidImageFormatException,
      );

      await expect(validateImageFile('/path/to/invalid.txt')).rejects.toThrow(
        'Invalid image format: Input buffer contains unsupported image format',
      );
    });
    ...
    it('should throw InvalidImageFormatException for non-Error exceptions', async () => {
      ...
      await expect(validateImageFile('/path/to/corrupted.jpg')).rejects.toThrow(
        InvalidImageFormatException,
      );

      await expect(validateImageFile('/path/to/corrupted.jpg')).rejects.toThrow(
        'Invalid image format: Unable to read image metadata',
      );
    });
  });
});
